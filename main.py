...
def segment_directory(path, threshold):
    ...
    for index, dir in enumerate(segmenter(directory['files'], threshold)):
        ...
        temp_parent_path = parent_path + new_subdir_name
        source = temp_parent_path
        make_directory(source)
        for file in dir:
            destination = f"{temp_parent_path}\{file.get_name()}"
            move_file(file.get_path(), destination)
...
def distribute_subdirs(directories, destination):
    ...
    for folder, files in directories.items():
        for file in files:
            move_file(file.get_path(), f"{Path(destination).parent}\{folder}")
...
def task_two_single(source, destination):
    ...
    :return: None
    """
    directories = get_subdirs_dict(destination)
    distribute_subdirs(directories, source)
    unpack_archives(source, source, archive_format)
    remove_files(source, archive_format)
...
def task_two(source, destination):
    ...
    progress = tqdm(np.arange(len(directory['files'])), desc="Loading")
    for subdir in directory['files']:
        task_two_single(subdir.get_path(), destination)
        progress.update()

task_one("D:\Xina\Test\Test80", "D:\movehere", 500000)
task_two("D:\Xina\Test\Test80", "D:\movehere")
